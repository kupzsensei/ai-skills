...
async function uninstall(skillName, flags) {
  ...
  // Remove auto-activation from the agent's instruction file
  const removedFrom = removeAutorun(skillName, agent, flags.project);
  if (removedFrom) {
    console.log(`  ✓ Auto-activation removed from ${removedFrom}`);
  }
}
