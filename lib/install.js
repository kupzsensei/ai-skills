...
const { applyRecommendedSettings } = require("./settings");
...
async function install(skillName, flags) {
  ...
  for (const skill of toInstall) {
    ...
    // Inject AUTORUN.md into the agent's instruction file if present
    const injectedTo = injectAutorun(skill, srcDir, agent, flags.project);
    if (injectedTo) {
      console.log(`  ✓ Auto-activation added → ${injectedTo}`);
    }
  ...
  // Offer to apply recommended settings
  if (count > 0) {
    await applyRecommendedSettings(agent.name, flags.project, flags.force);
  }
}
